...
export type ComponentKind = "field" | "validation" | "options";
export type ComponentUsageLocation = "field-origin" | "validation" | "options";
...
export interface ComponentUsage { componentId: string; sourceUri: string; sourcePath: string; location: ComponentUsageLocation; screenId?: string; fieldId?: string; referrerComponentId?: string }
export interface ComponentImpact { componentId: string; screenId: string; fieldId: string }
export interface ComponentUsageGraph { components: ComponentDefinition[]; usages: ComponentUsage[]; impacts: ComponentImpact[]; diagnostics: Diagnostic[] }

const KIND_BY_GROUP: Record<string, ComponentKind> = { fields: "field", validations: "validation", options: "options" };
...
function componentTarget(ref: string, sourceUri: string): { id: string; location: ComponentUsageLocation } | undefined {
  ...
  const match = target.hash.match(/^#(\/components\/(fields|validations|options)\/[^/]+)$/);
  if (!match) return undefined;
  const location = match[2] === "fields" ? "field-origin" : match[2] === "validations" ? "validation" : "options";
  target.hash = "";
...
export function buildComponentUsageGraph(documents: ProjectSpecDocument[]): ComponentUsageGraph {
  ...
  for (const component of components) if (!impacts.some((impact) => impact.componentId === component.id)) diagnostics.push({ severity: "warning", code: "unused-component", message: `Component "${component.name}" はどのFieldにも到達しません。`, where: component.id });
  return { components, usages, impacts, diagnostics };
}
