...
Viewerは画面コンテキストを維持し、Field詳細と同じドロワー内でComponent詳細、依存先、現在画面と他画面の利用箇所を表示する。Component Contractを第一目的とし、種別ごとの構造化表示を主、Authored YAMLを折りたたみの補助表示とする。Optionsは10件以下を全件表示し、10件を超える場合は検索可能な折りたたみ領域にする。依存ComponentはContract内の該当位置と依存一覧から個別に開き、自動展開しない。

上部の指標はDirect Usage Count、Impacted Field Count、Dependency Countの3つに限定する。利用FieldはDirect Usageと依存経由を区別し、現在画面を常時表示、他画面を画面単位で折りたたむ。Component固有診断はContract直後に内容がある場合だけ表示する。Identityは短い文書名・種別・名前を主表示し、完全なURIとJSON Pointerはコピー可能にする。
