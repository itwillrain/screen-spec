...
Design Tour専用のstepや説明文は定義しない。画像の定義順、各画像内のMapping定義順をそのまま巡回順とし、名称・文言・詳細は対象Screen Elementから導出する。これにより通常の双方向リンクとチュートリアルの順序が乖離せず、将来EditorでMappingを並べ替えるだけでTourも更新できる。v0.1は矩形だけを扱い、polygon、OCR、自動紐付け、画像上での編集は対象外とする。
