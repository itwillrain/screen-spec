...
function UIInstanceDetail({ instance, fieldKey, screen, headingRef, onClose, onOpenComponent, drawerWidth, onResizeStart, onResizeKeyDown }: {
  ...
  return <section className="field-detail" role="complementary" aria-labelledby="instance-detail-title" style={{ "--field-drawer-width": drawerWidth + "px" } as React.CSSProperties}>
    ...
    <dl className="field-detail-grid">{fieldKey ? <><Detail label="型" value={typeof fieldRecord?.type === "string" ? fieldRecord.type : undefined} code />{fieldRecord?.text !== undefined ? <Detail label="文言" value={String(fieldRecord.text)} /> : null}{typeof fieldRecord?.required === "boolean" ? <Detail label="必須" value={fieldRecord.required ? "必須" : "任意"} /> : null}{fieldRecord && "default" in fieldRecord ? <Detail label="既定値" value={displayValue(fieldRecord.default)} code /> : null}</> : null}<Detail label="Section" value={placement?.title ?? placement?.id} /><Detail label="Region" value={placement?.region ?? "body"} /><Detail label="Field Binding" value={String(fieldBindings.length)} /><Detail label="Event Mapping" value={String(fieldEvents.length)} /></dl>
