...
function TypeLabel({ value }: { value: string }) {
  return <span className="badge badge-region">{value}</span>
}
...
function UIInstanceDetail({ instance, fieldKey, screen, headingRef, onClose, drawerWidth, onResizeStart, onResizeKeyDown }: {
  ...
  return <section className="field-detail" role="complementary" aria-labelledby="instance-detail-title" style={{ "--field-drawer-width": drawerWidth + "px" } as React.CSSProperties}>
    ...
    <header><div><p className="eyebrow">{fieldKey ? "component field" : "component instance"}</p><h2 id="instance-detail-title" ref={headingRef} tabIndex={-1}><code>{fieldKey ? instance.key + "." + fieldKey : instance.key}</code>{fieldKey ? " — " + String(fieldRecord?.label ?? fieldKey) : null}</h2><p className="component-identity">{component?.name ?? componentName(instance.componentId ?? instance.componentRef ?? "Unknown")}</p></div><DrawerIconButton name="close" label="詳細を閉じる" onClick={onClose} /></header>
    <dl className="field-detail-grid">{fieldKey ? <><Detail label="型" value={typeof fieldRecord?.type === "string" ? fieldRecord.type : undefined} badge />{fieldRecord?.text !== undefined ? <Detail label="文言" value={String(fieldRecord.text)} copy /> : null}{typeof fieldRecord?.required === "boolean" ? <Detail label="必須" value={fieldRecord.required ? "必須" : "任意"} /> : null}{fieldRecord && "default" in fieldRecord ? <DefaultValueDetail value={fieldRecord.default} /> : null}</> : null}<Detail label="Section" value={placement?.title ?? placement?.id} /><Detail label="Region" value={placement?.region ?? "body"} /><Detail label="Field Binding" value={String(fieldBindings.length)} /><Detail label="Event Mapping" value={String(fieldEvents.length)} /></dl>
    ...
    {fieldBindings.length ? <section><h3>Field Bindings</h3><ul className="instance-mappings">{fieldBindings.map((binding) => <li key={binding.target}><code>{binding.target}</code><span aria-hidden="true"> ← </span>{binding.source ? <code>{binding.source}</code> : <code>{displayValue(binding.value)}</code>}</li>)}</ul></section> : null}
...
function FieldDetail({ item, section, headingRef, onClose, onOpenTab, onOpenComponent, componentUsages, drawerWidth, onResizeStart, onResizeKeyDown }: {
  ...
  return <section className="field-detail" role="complementary" aria-labelledby="field-detail-title" style={{ '--field-drawer-width': drawerWidth + 'px' } as React.CSSProperties}>
    ...
    <header><div><p className="eyebrow">selected field</p><h2 id="field-detail-title" ref={headingRef} tabIndex={-1}><code>{field.key}</code> — {field.label}</h2></div><DrawerIconButton name="close" label="Field詳細を閉じる" onClick={onClose} /></header>
    <dl className="field-detail-grid">
      <Detail label="型" value={field.type} badge />
...
function ComponentDetail({ componentId, screen, fieldId, headingRef, onBack, onClose, onOpenComponent, onNavigateField, drawerWidth, onResizeStart, onResizeKeyDown }: {
  ...
  return <section className="field-detail" role="complementary" aria-labelledby="component-detail-title" style={{ "--field-drawer-width": drawerWidth + "px" } as React.CSSProperties}>
    ...
    <header><div className="detail-heading-with-back"><DrawerIconButton name="back" label="元の詳細へ戻る" onClick={onBack} /><div><p className="eyebrow">{component.kind} component</p><h2 id="component-detail-title" ref={headingRef} tabIndex={-1}>{component.name}</h2><p className="component-identity"><code>{component.uri.split("/").pop()}</code><span aria-hidden="true"> › </span><code>{component.kind}</code><span aria-hidden="true"> › </span><code>{component.name}</code> <DrawerIconButton name="copy" label="Component参照をコピー" onClick={() => void navigator.clipboard?.writeText(component.id)} /></p></div></div><div className="detail-actions"><DrawerIconButton name="close" label="詳細を閉じる" onClick={onClose} /></div></header>
    ...
    <section><h3>定義</h3>
      {component.kind === "field" && contract ? <><dl className="field-detail-grid">{typeof contract.label === "string" ? <Detail label="ラベル" value={contract.label} /> : null}{typeof contract.type === "string" ? <Detail label="型" value={contract.type} badge /> : null}{typeof contract.text === "string" ? <Detail label="文言" value={contract.text} copy /> : null}{typeof contract.required === "boolean" ? <Detail label="必須" value={contract.required ? "必須" : "任意"} /> : null}{contract.default !== undefined ? <DefaultValueDetail value={contract.default} /> : null}{typeof contract.placeholder === "string" ? <Detail label="プレースホルダー" value={contract.placeholder} /> : null}</dl>{validations.length ? <section><h4>Validation</h4><div className="component-links">{dependencies.filter((usage) => usage.location === "validation").map((usage) => dependencyButton(usage.componentId, "validation"))}</div>{validations.filter((item) => !asRecord(item)?.$ref).map((item, index) => <pre key={index} className="contract-inline"><code>{stringifyYaml(item)}</code></pre>)}</section> : null}{contract.options !== undefined ? <section><h4>Options</h4><div className="component-links">{dependencies.filter((usage) => usage.location === "options").map((usage) => dependencyButton(usage.componentId, "options"))}</div></section> : null}</> : null}
...
function EventDetail({ event, onOpenApi }: { event: EventView; onOpenApi: () => void }) {
  return <article className="related-event event-flow"><header><div><p className="eyebrow">event</p><h4><code>{event.key}</code></h4></div><span className="event-route"><code>{event.from ?? "?"}</code> → {event.branches.length ? String(event.branches.length) + "分岐" : <code>{event.to ?? "?"}</code>}</span></header><div className="event-flow-step"><strong>1. Trigger</strong><p>{event.target ? <>Field <code>{event.target}</code></> : event.trigger ? <code>{event.trigger}</code> : <span className="muted">未定義</span>}</p></div>{event.context.length ? <div className="event-flow-step"><strong>2. Event Context</strong><dl className="event-context-list">{event.context.map((item) => <div key={item.name}><dt><code>event.{item.name}</code></dt><dd><code>{item.type}</code>{item.description ? " — " + item.description : null}</dd></div>)}</dl></div> : null}{event.branches.length ? <div className="event-flow-step"><strong>3. Branches（記述順）</strong><ol className="branch-list">{event.branches.map((branch) => <BranchDetail key={branch.id} branch={branch} onOpenApi={onOpenApi} />)}</ol></div> : <><div className="event-flow-step"><strong>3. Action</strong><p>{event.apiCall ? <button type="button" className="link" onClick={onOpenApi}>API <code>{event.apiCall}</code></button> : <span className="muted">なし</span>}</p></div><EventOutcomeSummary label="4. Result" outcome={{ to: event.to, expects: event.expects }} /><EventOutcomeSummary label="5. Success" outcome={event.onSuccess} /><EventOutcomeSummary label="5. Error" outcome={event.onError} /></>}</article>
}
